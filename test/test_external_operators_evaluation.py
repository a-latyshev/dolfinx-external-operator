    ...
    def psi_external(derivatives):
        if derivatives == (0, 0):
            ...
            def eval_op(I1_vals, slope_vals):
                # Operands that are real functions are projected/broadcasted to
                # the whole space (matching the shape of entities and evaluation
                # points), but use zero-stride views to avoid any memory
                # reallocation/copying. Slicing with [..., i] accesses
                # components of this strided view directly. Note: Avoid
                # operations like `slope_vals.reshape(-1)` or
                # `slope_vals.ravel()`, as flattening/reshaping zero-stride
                # dimensions cannot be represented as a view and will trigger a
                # full memory copy/allocation.
            ...
            def eval_deriv(I1_vals, slope_vals):
                # Operands that are real functions are projected/broadcasted to
                # the whole space (matching the shape of entities and evaluation
                # points) using zero-stride views to avoid memory
...
def test_mixed_solution_operands_on_quadrature_space():
    ...
    Q = fem.functionspace(
        domain,
        basix.ufl.quadrature_element(domain.topology.cell_name(), degree=2, value_shape=()),
    )
    ...
    def N_impl(grad_u, temp):
        # N = tr(grad(u)) + T
        return np.einsum("...ii->...", grad_u).reshape(-1) + temp.reshape(-1)

    def dN_dgradu_impl(grad_u, temp):
        # dN/d(grad(u)) = Identity
        n_pts = grad_u.shape[0] * grad_u.shape[1]
        identity = np.eye(gdim, dtype=grad_u.dtype)
        return np.tile(identity.reshape(1, -1), (n_pts, 1)).reshape(-1)

    def dN_dtemp_impl(grad_u, temp):
        # dN/dT = 1
        return np.ones(temp.size, dtype=temp.dtype)
    ...
    dx = Measure("dx", metadata={"quadrature_degree": 2})
    F = N * vT * dx + inner(grad(u), grad(vu)) * dx
    F_explicit = (ufl.tr(grad(u)) + T) * vT * dx + inner(grad(u), grad(vu)) * dx
